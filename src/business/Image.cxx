...
namespace Alder
{
  ...
  std::string Image::GetFileName()
  {
    ...
    if (!directory->Open(path.c_str()))
    {
      ...
      error << "Tried to get image file but the path \""
            << path
            << "\" does not exist.";
    ...
    std::stringstream error;
    error << "Tried to get image file in \""
          << path
  ...
  bool Image::IsRatedBy(User* user)
  {
    ...
    vtkNew<Rating> rating;
    if (!rating->Load(loader))
      return false;
  ...
  bool Image::SwapExamSideTo(const std::string& side)
  {
    ...
    if (Exam::SideStatus::Swappable == status)
    {
      ...
      if (0 < childCount && isParent)
      {
        ...
        {
          // swap the children
          std::vector<vtkSmartPointer<Image>>::iterator cit =
            childList.begin();
          std::vector<vtkSmartPointer<Image>>::iterator sit =
            siblingChildList.begin();
          do
          {
            Image* a = *cit;
            Image* b = *sit;
            ...
            vtksys::SystemTools::CopyFileAlways(
              aFile.c_str(), swapFile.c_str());
            vtksys::SystemTools::CopyFileAlways(
              bFile.c_str(), aFile.c_str());
            vtksys::SystemTools::CopyFileAlways(
              swapFile.c_str(), bFile.c_str());
            cit++;
            sit++;
    ...
    const int& rating, const bool& forward, const int& id)
  {
    ...
    // get neighbouring image which matches this image's exam type
    // and the given rating
    ...
    stream << "SELECT Image.Id "
           ...
           << "WHERE ScanType.Type = "
           << query->EscapeString(exam->GetScanType()) << " "
           << "AND Exam.Side = "
           << query->EscapeString(exam->Get("Side").ToString()) << " "
    ...
    if (query->NextRow())
    {
      ...
      if (!found)
        throw std::runtime_error(
          "Cannot find current atlas image in database.");
    }
  ...
  vtkSmartPointer<Image> Image::GetAtlasImage(const int& rating)
  {
    ...
    // get any image rated by an expert user having
    // the given exam type and rating score
    std::stringstream stream;
    stream << "SELECT Image.Id "
           << "FROM Image "
           << "JOIN Exam ON Image.ExamId = Exam.Id "
  ...
  bool Image::CleanHologicDICOM()
  {
    ...
    do
    {
      int val = static_cast<int>(
        image->GetScalarComponentAsFloat(ix++, iy, 0, 0));
      if (255 == val)
      {
        found = true;
        ix--;
