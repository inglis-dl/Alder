...
//
// -+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-
QReportDialogPrivate::QReportDialogPrivate(QReportDialog& object)
...
void QReportDialogPrivate::updateUi()
{
  ...
  { // for every user, add a new row
    ...
    for (auto mIt = modalityList.begin(); mIt != modalityList.end(); ++mIt)
    {
      std::string name = (*mIt)->Get("Name").ToString();
      item = new QTableWidgetItem;

      // does this user have any ratings associated with the current modality?

      std::map<std::string, int> ratings =
        Alder::Rating::GetNumberOfRatings(user);
      ...
      item->setText(
        hasModalityRating ?
          QString::number(ratings.find(name)->second) : QString("NA"));
      this->userTableWidget->setItem(
        0, this->columnIndex[QString(name.c_str())], item);
    }
  }
...
void QReportDialogPrivate::modalitySelectionChanged(QTableWidgetItem* item)
{
  ...
  if (hasSmtp)
  {
    ...
    {
      for (int row = 0;
           row < this->userTableWidget->rowCount() && !enableSmtp; ++row)
      {
        for (int col = 0;
             col < this->userTableWidget->columnCount(); ++col)
        {
...
{
  // NOTE: currently the columns with checkboxes cannot be sorted.
  // In order to do this we would need to either override
  // QSortFilterProxyModel::lessThan() or QAbstractTableModel::sort()
...
void QReportDialogPrivate::send()
{
  ...
  if (this->buildReportFile())
  {
    ...
    {
      std::string err = "Error: failed to mail rating report ";
      err += this->currentReportFileName.toStdString();
      err += "\nMail sender info: ";
    ...
    if (user->Load("Name", name))
    {
      ...
      for (auto vit = modalityVec.begin(); vit != modalityVec.end(); ++vit)
      {
        try
        {
          ...
          this->setProgress(static_cast<int>(progressScale*step++));
        }
