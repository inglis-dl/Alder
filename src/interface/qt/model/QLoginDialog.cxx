...
void QLoginDialog::accepted()
{
  ...
  {
    // login successful
    // if the password matches the default password, force the user to
    // change it
    ...
    while (defPassword == password)
    {
      ...
      QString password1 = QInputDialog::getText(
        ...
        QDialog::tr("Please provide a new password (cannot be \"password\")"\
                    "for your account:"),
        QLineEdit::Password);
    ...
    if (modalityList.empty())
    {
      ...
      str += "Please contact the system administrator ";
      str += "for modality assignment.";
  ...
  { // login failed
    ...
    errorMessage.setText(
      tr("Invalid username or password, please try again."));
    errorMessage.exec();
  }
}
