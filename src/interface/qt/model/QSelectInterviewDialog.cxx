...
void QSelectInterviewDialogPrivate::setupUi(QWidget* widget)
{
  ...
  this->interviewTableWidget->setSelectionMode(
  ...
  QObject::connect(
    this->interviewTableWidget->horizontalHeader(),
...
void QSelectInterviewDialogPrivate::sort(int col)
{
  ...
  {
    this->sortOrder = Qt::AscendingOrder == this->sortOrder ?
...
// -+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-+#+-
void QSelectInterviewDialogPrivate::updateRow(
  const int& row, Alder::Interview* interview)
{
  // interview meta data
  ...
  QString strVisitDate =
    QString(interview->Get("VisitDate").ToString().c_str());
  QString strWave = "N/A";
  ...
  // examCount the number of exams of each modality
  // and whether they have been rated
  for (auto it = vecExam.begin(); it != vecExam.end(); ++it)
  {
    Alder::Exam* exam = it->GetPointer();
    QString name = exam->GetModalityName().c_str();

    // the modalities and hence exams permitted to this user
  ...
  for (QMap<QString, bool>::iterator it = updateItemText.begin();
    it != updateItemText.end(); ++it)
  {
    if (it.value())
    {
      QString name = it.key();
  ...
  if (this->searchText.isEmpty() || this->searchTextInUId(strUId))
  {
    ...
    item =
      this->interviewTableWidget->item(row, this->columnIndex.value("Site"));
    if (item)
      item->setText(strSite);
    item =
      this->interviewTableWidget->item(row, this->columnIndex.value("UId"));
    if (item)
    {
      item->setText(strUId);
    ...
    item =
      this->interviewTableWidget->item(
        row, this->columnIndex.value("VisitDate"));
    if (item)
      item->setText(strVisitDate);
    item =
      this->interviewTableWidget->item(row, this->columnIndex.value("Wave"));
    if (item)
      item->setText(strWave);

    // add all modalities to the table
    for (QMap<QString, QString>::iterator it = itemText.begin();
      it != itemText.end(); ++it)
    {
      item =
        this->interviewTableWidget->item(
          row, this->columnIndex.value(it.key()));
      if (item)
  ...
  { // for every interview, add a new row
    ...
    if (this->searchText.empty() || this->searchTextInUId(UId))
    {
      ...
      this->interviewTableWidget->setItem(
        0, this->columnIndex.value("Site"), item);

      // add UId to row
      item = new QTableWidgetItem;
      item->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);
      this->interviewTableWidget->setItem(
        0, this->columnIndex.value("UId"), item);

      // add visit date to row
      item = new QTableWidgetItem;
      item->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);
      this->interviewTableWidget->setItem(
        0, this->columnIndex.value("VisitDate"), item);

      // add wave to row
      item = new QTableWidgetItem;
      item->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);
      this->interviewTableWidget->setItem(
        0, this->columnIndex.value("Wave"), item);
...
void QSelectInterviewDialogPrivate::selectionChanged()
{
  QList<QTableWidgetSelectionRange> ranges =
    this->interviewTableWidget->selectedRanges();
...
void QSelectInterviewDialog::accepted()
{
  Q_D(QSelectInterviewDialog);
  QList<QTableWidgetSelectionRange> ranges =
    d->interviewTableWidget->selectedRanges();
