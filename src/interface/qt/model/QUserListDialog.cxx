...
void QUserListDialogPrivate::updateUi()
{
  ...
  { // for every user, add a new row
    ...
    // add the expert row
    Qt::CheckState check =
      0 < user->Get("Expert").ToInt() ? Qt::Checked : Qt::Unchecked;
    item = new QTableWidgetItem;
    item->setFlags(
...
{
  // NOTE: currently the columns with checkboxes cannot be sorted.  In order to
  // do this we would need to either override QSortFilterProxyModel::lessThan()
  // or QAbstractTableModel::sort()
...
void QUserListDialog::remove()
{
  ...
  vtkNew<Alder::User> user;
  user->Load(
    "Id", items.at(d->columnIndex["Name"])->data(Qt::UserRole).toInt());
  // check if the user has ratings
  std::map<std::string, int> ratings =
    Alder::Rating::GetNumberOfRatings(user.GetPointer());
  ...
  if (hasRatings)
  {
    ...
    QMessageBox::information(
      this, QDialog::tr("Remove User"), stream.str().c_str());
  }
  else
  {
    stream << "Are you sure you wish to remove user \""
           << user->Get("Name").ToString()
           << "\"?  "
           << "This operation cannot be undone.";
    if (QMessageBox::Yes == QMessageBox::question(
        this, QDialog::tr("Remove User"),
        stream.str().c_str(), QMessageBox::Yes|QMessageBox::No))
    {
      user->Remove();
      d->updateUi();
    }
...
void QUserListDialog::add()
{
  ...
  QString name = QInputDialog::getText(
    this, QDialog::tr("Create User"),
    QDialog::tr("New user's name:"), QLineEdit::Normal);
  ...
  if (!name.isEmpty())
  {
    ...
    if (user->Load("Name", str))
    {
      std::stringstream stream;
      stream << "Unable to create new user \""
             << str
             << "\", name already in use.";
