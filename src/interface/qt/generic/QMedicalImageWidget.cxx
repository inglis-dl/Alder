  ...
  : QWidget(parent)
{
  ...
  connect(this->ui->foregroundButton, SIGNAL(clicked()),
    this, SLOT(slotSelectColor()));
  connect(this->ui->backgroundButton, SIGNAL(clicked()),
    this, SLOT(slotSelectColor()));
  ...
  int iconSize =
    this->ui->backgroundButton->style()->pixelMetric(QStyle::PM_SmallIconSize);
  QPixmap pix(iconSize, iconSize);
  pix.fill(
    qcolor.isValid() ?
    this->ui->backgroundButton->palette().button().color() : Qt::transparent);
  ...
  pix.fill(
    qcolor.isValid() ?
    this->ui->foregroundButton->palette().button().color() : Qt::transparent);
  painter.setBrush(qcolor.isValid() ? qcolor : QBrush(Qt::NoBrush));
  painter.drawRect(2, 2, pix.width()-2, pix.height()-2);
...
bool QMedicalImageWidget::eventFilter(QObject* obj, QEvent* event)
{
  if (obj == this->ui->vtkWidget)
  {
    ...
    {
      qobject_cast<QFrame*>(this->ui->vtkWidget->parent())->setStyleSheet(
...
void QMedicalImageWidget::slotSelectColor()
{
  ...
  if (button && dialog.exec())
  {
    ...
    QPixmap pix(iconSize , iconSize);
    pix.fill(
      qcolor.isValid() ? button->palette().button().color() : Qt::transparent);
    QPainter painter(&pix);
    painter.setPen(QPen(Qt::gray));
    painter.setBrush(qcolor.isValid() ? qcolor : QBrush(Qt::NoBrush));
    painter.drawRect(2, 2, pix.width()-2, pix.height()-2);
