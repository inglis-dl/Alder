...
int main(int argc, char** argv)
{
  ...
  {
    std::cout
      << "Usage: OpalHttp <username> <password> <host> <port> <ws> [outfile]"
      << endl
      << "Example:"
      << endl
      << "  OpalHttp username password localhost 8843 <ws> output.jpg"
      << endl
      << "  Where <ws> can be any of the following:"
      << endl
      << "    /datasources"
      << endl
      << "      All datasources"
      ...
      << endl
      << "      All entities of a table matching a script"
      << endl
      << "      (see http://wiki.obiba.org/display/OPALDOC/Magma+Javascript+API)"
      << endl
      << endl
  ...
  if (curl)
  {
    ...
    if (0 != res)
      cout << "Error (Code " << res << "): " << curl_easy_strerror(res) << endl;
    else
    {
      if (toFile)
        cout << "Success, result written to \"" << outfile << "\"" << endl;
      else
        cout << output << endl;
    }
